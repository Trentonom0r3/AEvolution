...
AEvolution_API LayerH createSolidInComp(const std::string& name, std::tuple<int, int> dimensions, std::tuple<float, float, float, float> color, CompH parentCompH, float dur);

AEvolution_API LayerH createCameraInComp(const std::string& name, std::pair<float, float> position, CompH parentCompH);
