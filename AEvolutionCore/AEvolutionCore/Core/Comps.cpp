...
colorH GetCompBGColor(CompH compH)
{
	...
	try {
		...
		colorH color = *boost::get<boost::shared_ptr<colorH>>(resp.args[0]); // Extract the color from the response
...
void SetCompBGColor(CompH compH, colorH color)
{
	auto& mqm = MessageQueueManager::getInstance();
	try {
		Command cmd(createUUID(), "SetCompBGColor", CommandArgs{ boost::make_shared<CompH>(compH), boost::make_shared<colorH>(color) }); // Command to be sent to the server
		mqm.sendCommand(cmd);
